...
export default function LeftPanel() {
  ...
  return (
    <aside style={{
      width: 260, background: 'var(--bg-panel)', borderRight: '1px solid var(--border-panel)',
      display: 'flex', flexDirection: 'column', flexShrink: 0, zIndex: 20, overflow: 'hidden',
    }}>
      <nav style={{ display: 'flex', borderBottom: '1px solid var(--border-panel)', flexShrink: 0 }}>
        {tabs.map((tab) => {
          const isActive = activeTab === tab.id
          return (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{
              flex: 1, padding: '7px 4px', fontSize: 8,
              fontFamily: 'var(--font-mono)', fontWeight: 700, letterSpacing: '0.06em',
              color: isActive ? 'var(--amber-primary)' : 'var(--text-dim)',
              background: isActive ? 'var(--amber-glow)' : 'transparent',
              border: 'none', borderBottom: isActive ? '2px solid var(--amber-primary)' : '2px solid transparent',
      ...
      <AnimatePresence mode="wait">
        <motion.div key={activeTab} style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}
